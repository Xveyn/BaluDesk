...
### 4. Build the Backend
```powershell
cd "F:\Programme (x86)\BaluDesk\backend"
.\build.bat release
```
...
### "No C++ compiler found (g++ or clang++)" on Windows

**Problem**: You tried to run `./build.sh` or `bash build.sh` on Windows.

**Solution**: Windows requires `build.bat` instead:
```cmd
build.bat release
```

**Why**: `build.sh` is designed for Unix systems (Linux/macOS) and checks for Unix compilers (g++/clang++) which aren't available on standard Windows installations. Windows uses MSVC (Visual Studio compiler) which requires `build.bat`.

**Alternative - MSYS2/MINGW**: If you want to use `build.sh` on Windows via MSYS2:
...
3. Install compiler and dependencies:
   ...
   pacman -S mingw-w64-x86_64-curl mingw-w64-x86_64-sqlite3
   ```
4. Build:
   ```bash
   cd "/path/to/baludesk/backend"
   ./build.sh Release
   ```
...
**Windows**:
Download from https://cmake.org/download/ or use:
```cmd
winget install -e --id Kitware.CMake
```

### vcpkg Dependencies Failing (Windows)

If `.\vcpkg install curl:x64-windows sqlite3:x64-windows` fails:

1. **Check Visual Studio**: Ensure "Desktop development with C++" workload is installed
2. **Update vcpkg**: `git pull` in vcpkg directory
3. **Clean install**:
   ```cmd
   .\vcpkg remove curl:x64-windows sqlite3:x64-windows
   .\vcpkg install curl:x64-windows sqlite3:x64-windows --recurse
   ```

### libcurl/sqlite3 Not Found (Linux)
...
```bash
# Linux/macOS
ls -la ./build/baludesk-tests

# Windows
dir .\build\Release\baludesk-tests.exe
```

If missing, check CMake output for compilation errors.
